...
class PickerPorteAnimal: UIPickerView,UIPickerViewDataSource, UIPickerViewDelegate {
    
    
    var textField:UITextField?
    var controller:UIViewController?
    var rowSelected = -1
    var enumSelected:EnumPorteAnimal?
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        ...
        rowSelected = row
        enumSelected = EnumPorteAnimal.getEnum(orderId: rowSelected)
        textField?.text = enumSelected?.rawValue
    }
    
    
    func prepare(textField:UITextField, controller:UIViewController,enumTp:EnumPorteAnimal?) -> PickerPorteAnimal{
        self.delegate = self
        ...
        var countEnum:Int = 0
        if(enumTp != nil ){
            for f in EnumUtil.iterateEnum(EnumPorteAnimal.self) {
